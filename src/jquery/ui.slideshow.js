	...
	$.widget('ui.slideshow', {
		...
		*/
		_create: function(){
			...
			this.classes.get = $.proxy(function(){
			...
			if (!data){
				this.options.hu = '';
				data = {};
				var thumbnails = this.element.find(this.classes.get('thumbnails') + ' img');
				this.element.find(this.classes.get('images') + ' img').each(function(i){
					var src = this.src,
						caption = this.alt || this.title,
						href = $(this).parent().attr('href'),
						thumbnail = thumbnails.get(i) ? thumbnails.get(i).src : '';
			...
			for (action in this.options.accesskeys){
				...
				['shift', 'control', 'alt'].each(function(modifier){
					...
					accesskey[modifier] = re.test(obj.key);
